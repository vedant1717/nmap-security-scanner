document.addEventListener('DOMContentLoaded', () => {
    ...
    const scanTypeDesc = document.getElementById('scan-type-desc');
    
    const descriptions = {
        'port': 'Port Scan: Quickly discovers all open TCP ports on the host.',
        'script': 'Script Scan: Runs Nmap version detection and default safe NSE scripts on discovered ports.',
        'full': 'Full Scan: Scans the entire 65,535 TCP port range and runs script scans on discovered ports.',
        'udp': 'UDP Scan: Scan for open UDP ports (requires sudo privileges).',
        'vulns': 'Vulns Scan: Checks discovered ports against CVE vulnerability databases and runs vuln scripts.',
        'recon': 'Recon Scan: Runs custom recommendation recon scripts based on discovered ports.',
        'all': 'All Scans: Runs every single scan module consecutively (Port, Script, Full, UDP, Vulns, Recon).',
        'network': 'Network Scan: Identifies active live hosts on the target subnet.'
    };

    if (scanTypeSelect) {
        scanTypeSelect.addEventListener('change', () => {
            const val = scanTypeSelect.value;
            scanTypeDesc.textContent = descriptions[val] || '';
        });
    }
    ...
    function updateProgress(data) {
        ...
        if (status === 'completed' || status === 'aborted') {
            ...
            downloadBtn.onclick = () => {
                window.location.href = `/api/download_automator/${currentJobId}`;
            };
        ...
        if (results.length > currentRowsCount) {
            for (let i = currentRowsCount; i < results.length; i++) {
                ...
                if (res.open_ports.includes('Error') || res.open_ports.includes('aborted') || res.open_ports.includes('down') || res.open_ports.includes('unresolvable')) {
                ...
                if (res.findings === 'Clean') {
                    findingsTd.innerHTML = `<span class="badge badge-info">Clean</span>`;
                } else {
                    findingsTd.innerHTML = parseFindings(res.findings);
                }
                ...
                dlBtn.onclick = () => {
                    window.location.href = `/api/download_automator_raw/${currentJobId}/${res.ip}`;
                };
    ...
    function parseFindings(findingsStr) {
        ...
        return items.map(item => {
            ...
            if (item.includes('CRITICAL') || item.includes('Expired') || item.includes('VULNERABLE') || item.includes('CVE')) className = 'badge-critical';
            ...
            // Clean up the text by removing tags
